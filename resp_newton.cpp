...
//        H_xy = real(-mtimesx(ky_exp_ky, mtimesx(responsef, kx_exp_kx, 'speed'), 'speed'));
        vector <Mat> H_xy_temp2;
        H_xy_temp2 = QT_mtimesx(ky_exp_ky,QT_mtimesx(responsef,kx_exp_kx));
        vector <Mat> H_xy;
...
//            subtract(max_pos_y.at(n),c,d);
            ...
            Mat output;
            divide((max_pos_y.at(n) - ((H_xx.at(n).mul(grad_y.at(n))) - (H_xy.at(n).mul(grad_x.at(n))))),det_H.at(n),output);
...
//            subtract(max_pos_x.at(n),c,d);
            ...
            Mat output;
            divide((max_pos_x.at(n) - ((H_yy.at(n).mul(grad_x.at(n))) - (H_xy.at(n).mul(grad_y.at(n))))),det_H.at(n),output);
...
//    max_response = 1 / prod(use_sz) * real(mtimesx(mtimesx(exp_iky, responsef, 'speed'), exp_ikx, 'speed'));
    ...
    for(int n = 0 ; n < responsef.size() ; ++n){
        ...
        mtimesx_temp2 = QT_mtimesx( QT_mtimesx(exp_iky,responsef),exp_ikx);
