...
void* alMalloc( size_t size, int alignment ) {cout<<"10_alMalloc"<<endl;
...
void alFree(void* aligned) {cout<<"19_alFree"<<endl;
...
void grad1( float *I, float *Gx, float *Gy, int h, int w, int x ) {cout<<"34_grad1"<<endl;
...
void grad2( float *I, float *Gx, float *Gy, int h, int w, int d ) {cout<<"59_grad2"<<endl;
...
float* acosTable() {cout<<"66_acosTable"<<endl;
...
void gradMag( float *I, float *M, float *O, int h, int w, int d, bool full ) {cout<<"78_gradMag"<<endl;
    cout<<"79_gradMag"<<endl;
  ...
  for( x=0; x<w; x++ ) {
    ...
    for( y=0; y<h4/4; y++ ) {
      _m = sse::min1( sse::RCPSQRT(_M2[y]), sse::SET(1e10f) );
      _M2[y] = sse::RCP(_m);
...
void gradMagNorm( float *M, float *S, int h, int w, float norm ) {cout<<"126_gradMagNorm"<<endl;
...
float* hogNormMatrix( float *H, int nOrients, int hb, int wb, int bin ) {cout<<"259_hogNormMatrix"<<endl;
...
float* crop_H(float *H,int* h_height,int* h_width,int depth,int dh,int dw){cout<<"441_crop_H"<<endl;
...
float* fhog(float* I,int height,int width,int channel,int *h,int *w,int *d,int binSize, int nOrients, float clip, bool crop){cout<<"455_fhog"<<endl;
...
void change_format(float *des,float *source,int height,int width,int channel){cout<<"475_change_format"<<endl;
...
cv::Mat fhog(const cv::Mat& input, int binSize, int nOrients, float clip, bool crop){cout<<"482_fhog"<<endl;
    ...
    for (size_t i = 0; i < HEIGHT; i++) {
        for (size_t j = 0; j < WIDTH; j++) {
            ...
            for (size_t k = 0; k < DEPTH; k++) {
                II[count] = input.at<float>(i, j) / 255.f;
                // cout<<II[count]<<endl;
                count++;
            }
        }
